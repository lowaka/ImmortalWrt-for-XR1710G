...
#ifndef VLAN_ETH_ALEN
#define VLAN_ETH_ALEN ETH_ALEN
#endif
...
int hy_enable;
int hy_lan_count;
char hy_br_wan[PON_VLAN_ITF_NAME_SIZE];
char hy_lan[HYBRID_LAN_COUNT_MAX][PON_VLAN_ITF_NAME_SIZE];
...
static int handle_igmp_tag(struct sk_buff ** pskb,int mode, __u16 tci)
{
	...
	if(!((XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_IGMP) && strcmp(out_dev->name,"pon") == 0))	
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"vlan flag error or out_dev isn't nasx return -1====>Handle IGMP Tag");
		return -1;
	}
	...
	switch(mode)
	{
		case IGMP_MODE_PASS_THROUGH://do nothing
			PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"handle igmp packet with mode 0:transparent====>Handle IGMP Tag");
			return 0;

		case IGMP_MODE_ADD_TAG:
			PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"handle igmp packet with mode 1:add tag====>Handle IGMP Tag");
			goto add_tag;

		case IGMP_MODE_MODIFY_TCI:
		case IGMP_MODE_MODIFY_VID:
			PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"handle igmp packet with mode 2&3:modify tci or vid====>Handle IGMP Tag");
			if(XPON_SKB_CB(skb)->pon_tag_num == 0)
				goto add_tag;
			else
				goto modify_TCI;
		
		default:
			PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"mode error ====> Handle IGMP Tag");
			return -1;
	}

add_tag:
	XPON_SKB_CB(skb)->pon_vlan_tpid[XPON_SKB_CB(skb)->pon_tag_num] = ETH_P_8021Q;
	XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num] = newTCI;
	XPON_SKB_CB(skb)->pon_tag_num ++;
	PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"add tag success,tci is %x====>Handle IGMP Tag",XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num]);
	return 0;

modify_TCI:
	if(mode == IGMP_MODE_MODIFY_TCI)
		XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num - 1] = newTCI;
	else		// mode == IGMP_MODE_MODIFY_VID
		XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num - 1] = (XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num - 1] & 0xF000) | (newTCI & 0x0FFF);

	PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"modify tci success,tci is %x====>Handle IGMP Tag",XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num - 1]);
	return 0;
...
static int skb_mark_convert_port_num(struct sk_buff *skb)
{
	...
	int skb_mark2 = 0;	
	skb_mark = skb->mark;
	skb_mark2 = XPON_SKB_CB(skb)->mark2;
...
static inline int get_port_num(struct sk_buff *skb)
{
	...
	if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_CPE)
	{
		/* only support one ip host now */
		if(TCSUPPORT_PON_IP_HOST_VAL && (XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_VOIP_TX)){
	...
	else if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_LAN)
	{
		PONVLAN_PRINT(PONVLAN_MSG_TRACE,"packet from lan and mark is %x====>get port num",skb->mark);
		num = skb_mark_convert_port_num(skb);
		if(num > 0)
		{
			PONVLAN_PRINT(PONVLAN_MSG_TRACE|XPON_SKB_CB(skb)->pon_vlan_flag,"packet from lan num=%d ====>get port num",num);
			return num;
		}

		PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"packet from lan and can't find port====>get port num");
		return -1;
	}
	else if (XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_PPTP) //hybrid PPTP
	{
		num = skb_mark_convert_port_num(skb);
		if(num > 0)
		{
			PONVLAN_PRINT(PONVLAN_MSG_TRACE|XPON_SKB_CB(skb)->pon_vlan_flag,"packet from PPTP num=%d base on skb->mark",num);
			return num;
		}
		else{
			PONVLAN_PRINT(PONVLAN_MSG_TRACE|XPON_SKB_CB(skb)->pon_vlan_flag,"not find port! return -1");
			return -1;
		}
	}
	if(hy_enable == 1 && (XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK) && !memcmp(out_dev->name, "eth0.",5)) //hybrid downstream eth
	{
		for(i = 0; i < hy_lan_count; i++)
		{
			if(strcmp(out_dev->name, hy_lan[i]) == 0)
			{
				...
				{
					PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"packet from wan and can't find port====>get port num");
					return -1;
				}
			}
		}
	...
	else if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_WAN || XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_SFU_WAN)
	{
		...
		if(TCSUPPORT_PON_IP_HOST_VAL){
			if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_VOIP_RX)
				return PONVLAN_PORT_OFFSET_IPHOST;
		}
		num = devname_convert_port_num(out_dev->name);
		if(num > 0)
	...
	else if( (XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK) && (pon_vlan_all_data.onu_mode == MODE_HGU))
	{
		PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"packet from wan(hgu RX)\n");
		return PONVLAN_PORT_OFFSET_VP;
	}
	else if( (XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_TX_CALL_HOOK) && (pon_vlan_all_data.onu_mode == MODE_HGU))
	{
		PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"packet from wan(hgu TX)\n");
		return PONVLAN_PORT_OFFSET_VP;
	}
...
static inline pon_vlan * get_vlan_opt_data(struct sk_buff *skb)
{
	...
	if(pon_vlan_all_data.onu_mode == MODE_HGU){
		if((XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_VOIP_TX)
			|| (XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_VOIP_RX))
...
#else
	if(pon_vlan_all_data.onu_mode == MODE_HGU && 
		XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_VEIP)
	...
	if(port == -1)
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Get port error ====>get vlan opt data");
		return NULL;
	}

	dir = get_stream_direction(XPON_SKB_CB(skb)->pon_vlan_flag);
	for(i = 1; i < pon_vlan_all_data.total_port_count; i++)
	{
		...
		if(pon_vlan_all_data.pon_vlan_data[i].port_index == port)
		{
			if(dir == UPSTREAM)
			{
				...
				{
					PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"we have rule on port %d in upstream,====>get vlan opt data",port);
			...
			else if(dir == DOWNSTREAM)
			{
				...
				{
					PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"we have rule on port %d in downstream,====>get vlan opt data",port);
					return &pon_vlan_all_data.pon_vlan_data[i];
				}
				else
				{
					...
					{
						PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"wmode is transparent or stripped on port %d in downstream,====>get vlan opt data",port);
						return &pon_vlan_all_data.pon_vlan_data[i];
					}
			...
			{
				PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"lan 2 lan or other case,value is %d.return NULL ====>get vlan opt data",port);
				return NULL;
			}
		}
	}
	...
	if(flag != 1)
	{
		PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"unknow packet's port,value is %d.return NULL ====>get vlan opt data",port);
		return NULL;
	}
    ...
    {/* 247测试用例6.4 */    
		__u8 *mac = NULL;
        mac = skb->data;
        PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"TCSUPPORT_BBF_247_VAL, data in skb is %x %x %x %x %x %x \n",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
	...
	if(pon_vlan_all_data.pon_vlan_data[i].enable_default_rule == ENABLE)
	{
		if(dir == UPSTREAM)
		{
			...
			{
				PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"we will use default rule on port %d in upstream,====>get vlan opt data",
					pon_vlan_all_data.pon_vlan_data[offset].port_index);
				return &pon_vlan_all_data.pon_vlan_data[offset];
			}
			else
			{
				PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"No rule for port %d in upstream.return NULL ====>get vlan opt data",port);
				return NULL;
			}
		}
		else if(dir == DOWNSTREAM)
		{
			...
			{
				PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"we will use default rule on port %d in downstream,====>get vlan opt data",
					pon_vlan_all_data.pon_vlan_data[offset].port_index);
				return &pon_vlan_all_data.pon_vlan_data[offset];
			}
			else
			{
				PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"No rule for port %d downstream.return NULL ====>get vlan opt data",port);
				return NULL;
			}
		}
		else
		{
			PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"Unknown error for port %d downstream.return NULL ====>get vlan opt data",port);
			return NULL;
		}
	}
	else
	{
		PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"No rule on port %d and don't use default rule,return NULL ====>get vlan opt data",port);
		return NULL;
	}
	return NULL;
}
...
static inline int store_tag_info(struct sk_buff *skb)
{
	...
	for(i = 0; i < tag_limit; i++)
	{
		...
		if(XPON_SKB_CB(skb)->pon_tag_num > 0)
		{
			for(j = XPON_SKB_CB(skb)->pon_tag_num; j > 0; j--)
			{
				XPON_SKB_CB(skb)->pon_vlan_tpid[j] = XPON_SKB_CB(skb)->pon_vlan_tpid[j - 1];
				XPON_SKB_CB(skb)->pon_vlan_tci[j] = XPON_SKB_CB(skb)->pon_vlan_tci[j - 1];
			}
		}
		XPON_SKB_CB(skb)->pon_vlan_tpid[0] = htons(*tmp);
		XPON_SKB_CB(skb)->pon_vlan_tci[0] = htons(*(tmp+1));
		XPON_SKB_CB(skb)->pon_tag_num++;
		...
		PONVLAN_PRINT(PONVLAN_MSG_TRACE,"Tag Conut is %d",XPON_SKB_CB(skb)->pon_tag_num);
	}
	...
	/*if(is_vlan(tmp))
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"frame has 5 or more tags,return -1 ====>store tag info");
		return -1;
	}*/
...
static inline int match_tag(struct sk_buff * skb, pon_vlan_rule * rule, int tci_index, pon_vlan * data, int direction)
{
    ...
    int pbitMatchFlag = 1;
	int tag_index = tci_index + 1;
	__u16 tag_tpid = XPON_SKB_CB(skb)->pon_vlan_tpid[tci_index];
	__u16 tci = XPON_SKB_CB(skb)->pon_vlan_tci[tci_index];
	__u16 in_tpid = data->input_tpid;
	__u16 out_tpid = data->output_tpid;
	int vidMatchFlag = 1;
...
static inline int up_rule_tag0(struct sk_buff * skb, pon_vlan * data, int direction,int index)
{
	...
	if(rule->tag_num != 0)
	{
		PONVLAN_PRINT(PONVLAN_MSG_WARNING," skb tag num %d, rule's tag num is %d ====>pon match rule",XPON_SKB_CB(skb)->pon_tag_num,rule->tag_num);
		return 0;
	}
...
static inline int match_rule(struct sk_buff * skb, pon_vlan * data, int direction,int index)
{
	...
	{
		PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"rule's tag num is %d ,it will cause error====>pon match rule",rule->tag_num);
		return 0;
	}
	
	if(XPON_SKB_CB(skb)->pon_tag_num != rule->tag_num)
	{
		PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"frame has %d Tag,rule's tag num is %d ====>pon match rule",XPON_SKB_CB(skb)->pon_tag_num,rule->tag_num);
		return 0;
	}
	...
	if(rule->filter_ethertype != 0)
	{
		...
		{
			PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"Ether Type not match,frame etherType is %x,rule's EtherType is %x ====>pon match rule",ethertype,rule->filter_ethertype);
			return 0;
		}
	}
	...
	if(XPON_SKB_CB(skb)->pon_tag_num == 0){
		...
		for(i = XPON_SKB_CB(skb)->pon_tag_num - 1; i >= 0; i--){
			matchValue = match_tag(skb, rule, i, data, direction);
			if(matchValue == 0)	{
				PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"match tag %d error return 0 ====>pon match rule",i+1);
				return 0;
			}

			if(i < 2 && (direction == UPSTREAM ) && (RULE_MATCH_TOTAL != matchValue)){
				PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"upstream match total error return 0 ====>pon match rule");
				return 0;
			}
...
static inline int get_match_rule(struct sk_buff * skb, pon_vlan * vlan_data ,int dir)
{
    ...
    for(i = vlan_data->up_rule_count -1; i >= 0; i--) //search from tail
	{
		if(HIGH_PRIORITY == vlan_data->up_rule[i].rule_priority 
            ...
            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"match high pri rule idx = %d ====>pon match rule",i);
            return i;
        }
    ...
    if (INVALID_VLAN_RULE_NUM != idx_low_pir){
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"match low pri rule idx = %d ====>pon match rule",idx_low_pir);
        return idx_low_pir;
    }

    if (INVALID_VLAN_RULE_NUM != idx_default){
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"default rule idx = %d ====>pon match rule",idx_default);
        return idx_default;
    }

    PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"no match rule ====>pon match rule");
	    
	//if(has_default_rule || ((vlan_data->up_rule_count != 0 )&& (vlan_data->port_index != PONVLAN_PORT_OFFSET_DEFGPON))){
	if(has_default_rule ){
		PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"default not march");
...
static inline int write_tag_info_to_skb(struct sk_buff * skb,int index,int tpid,int pbit,int dei,int vid,__u16 in_tpid,__u16 out_tpid,__u8 * map,original_tag otag)
{
	switch(tpid)
	{
		case TREAT_TPID_8100:
			XPON_SKB_CB(skb)->pon_vlan_tpid[index] = ETH_P_8021Q;
			break;

		case TREAT_TPID_OUTPUT_TPID:
			XPON_SKB_CB(skb)->pon_vlan_tpid[index] = out_tpid;
			break;

		case TREAT_TPID_COPY_FROM_INNER:
			if(otag.tag_num == 1 || otag.tag_num == 2)
			{
				XPON_SKB_CB(skb)->pon_vlan_tpid[index] = otag.inner_tpid;
			}
			else
			{
				PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"TPID copy from inner error ====>Write Tag Info to Skb");
				return -1;
			}
			break;
		...
		case TREAT_TPID_COPY_FROM_OUTER:
			if(otag.tag_num == 2)
			{
				XPON_SKB_CB(skb)->pon_vlan_tpid[index] = otag.outer_tpid;
			}
			else
			{
				PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"TPID copy from outer error ====>Write Tag Info to Skb");
				return -1;
			}
			break;

		case TREAT_TPID_INPUT_TPID:
			XPON_SKB_CB(skb)->pon_vlan_tpid[index] = in_tpid;
			break;
			
		default:
			PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Vlan rule TPID error ====>Write Tag Info to Skb");
			return -1;
	}
	...
	switch(pbit)
	{
		...
		case 6:
		case 7:
			setPbit(XPON_SKB_CB(skb)->pon_vlan_tci[index],pbit);
			break;

		case TREAT_PRI_COPY_FROM_INNER:
			if(otag.tag_num == 1 || otag.tag_num == 2)
			{
				setPbit(XPON_SKB_CB(skb)->pon_vlan_tci[index],getPbit(otag.inner_tci));
			}
			else
			{
				PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Pbit copy from inner error ====>Write Tag Info to Skb");
				return -1;
			}
			break;
		...
		case TREAT_PRI_COPY_FROM_OUTER:
			if(otag.tag_num == 2)
			{
				setPbit(XPON_SKB_CB(skb)->pon_vlan_tci[index],getPbit(otag.outer_tci));
			}
			else
			{
				PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Pbit copy from outer error ====>Write Tag Info to Skb");
				return -1;
			}
			break;

		case TREAT_PRI_BASED_ON_DSCP:
			setPbit(XPON_SKB_CB(skb)->pon_vlan_tci[index],get_pbit_based_on_dscp(skb,map));
			break;

		default:
			PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Vlan rule Pbit error ====>Write Tag Info to Skb");
			return -1;
	}
	...
	switch (dei)
	{
		case 0:
		case 1:
			setDEI(XPON_SKB_CB(skb)->pon_vlan_tci[index],dei);
			break;

		case TREAT_DEI_COPY_FROM_INNER:
			if(otag.tag_num == 1 || otag.tag_num == 2)
			{
				setDEI(XPON_SKB_CB(skb)->pon_vlan_tci[index],getDEI(otag.inner_tci));
			}
			else
			{
				PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"DEI copy from inner error ====>Write Tag Info to Skb");
				return -1;
			}
			break;
		...
		case TREAT_DEI_COPY_FROM_OUTER:
			if(otag.tag_num == 2)
			{
				setDEI(XPON_SKB_CB(skb)->pon_vlan_tci[index],getDEI(otag.outer_tci));
			}
			else
			{
				PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"DEI copy from outer error ====>Write Tag Info to Skb");
				return -1;
			}
			break;

		default:
			PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Vlan rule DEI error ====>Write Tag Info to Skb");
			return -1;
	}
	...
	if(vid >= 0 && vid <= 4095)
	{
		setVID(XPON_SKB_CB(skb)->pon_vlan_tci[index],vid);
	}
	else if(vid == TREAT_VID_COPY_FROM_INNER)
	{
		if(otag.tag_num == 1 || otag.tag_num == 2)
		{
			setVID(XPON_SKB_CB(skb)->pon_vlan_tci[index],getVID(otag.inner_tci));
		}
		else
		{
			PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"VID copy from inner error ====>Write Tag Info to Skb");
			return -1;
		}
	}
	else if(vid == TREAT_VID_COPY_FROM_OUTER)
	{
		if(otag.tag_num == 2)
		{
			setVID(XPON_SKB_CB(skb)->pon_vlan_tci[index],getVID(otag.outer_tci));
		}
		else
		{
			PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"VID copy from outer error ====>Write Tag Info to Skb");
			return -1;
		}
	}
	else
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Vlan rule VID error ====>Write Tag Info to Skb");
		return -1;
	}
	return 0;
}
...
static inline int add_tag_to_skb(struct sk_buff * skb, pon_vlan_rule * rule, int tagIndex, __u16 in_tpid,__u16 out_tpid,__u8 * map,original_tag otag)
{
	...
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"paramater error ====>Add Tag to Skb");
		return -1;
	}

	pos = XPON_SKB_CB(skb)->pon_tag_num;
	XPON_SKB_CB(skb)->pon_vlan_tci[pos] = 0;//clean tag info
...
static inline int del_tag_from_skb(struct sk_buff * skb, int num)
{
	...
	if(num > XPON_SKB_CB(skb)->pon_tag_num || num == 0)
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Del num is %d,and packet Tag num is %d,it will cause an error====> Del Tag",num,XPON_SKB_CB(skb)->pon_tag_num);
		return -1;
	}

	PONVLAN_PRINT(PONVLAN_MSG_TRACE,"del_tag_from_skb, Del num is %d,and packet Tag num is %d",num,XPON_SKB_CB(skb)->pon_tag_num);
	for(i=0; i<num; i++){ //remove vlan  tag
		pos =  XPON_SKB_CB(skb)->pon_tag_num - num +i;
		if(pos>=0 && pos<4){
			XPON_SKB_CB(skb)->pon_vlan_tpid[pos] = 0;
			XPON_SKB_CB(skb)->pon_vlan_tci[pos] = 0;
		}
		else
			break;
	}

	XPON_SKB_CB(skb)->pon_tag_num -= num;
...
static inline int change_tag_in_skb(struct sk_buff * skb, pon_vlan_rule * rule, int tag_position, __u16 in_tpid, __u16 out_tpid,__u8 * map,original_tag otag,int matchFlag)
{
	...
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"paramater error ====>Change tag in skb");
		return -1;
	}
...
static inline int do_option(struct sk_buff * skb, pon_vlan * data, int direction,int index,original_tag otag,int matchFlag)
{
	...
	switch(i/10)
	{
		...
		case METHOD_ADD_TAG:
			...
			{
				PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Add first tag to skb error====> Do option");
				return -1;
			}
			...
			if(i == ADD_AND_CHANGE_OUTER_TAG)
			{
				...
				{
					PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Change tag in skb error(add case)====> Do option");
					return -1;
				}
			}
			...
			if(i%10 == ADD_SECOND_TAG)
			{
				...
				{
					PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Add second tag to skb error====> Do option");
					return -1;
				}
			}
			return 0;
		...
		case METHOD_DEL_TAG:
			if(i == DEL_AND_CHANGE_INNER_TAG)
			{
				...
				{
					PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Del tag from skb error====> Do option");
					return -1;
				}
				if(change_tag_in_skb(skb,rule,DEL_AND_CHANGE_INNER_TAG,data->input_tpid,data->output_tpid,data->dscp_map,otag,matchFlag) == -1)
				{
					PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Change tag in skb error(del case)====> Do option");
					return -1;
				}
			}
			else
		...
		case METHOD_CHANGE_TAG:
			if(i%10 == 0 || i%10 == 2)
			{
				...
				{
					PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Change inner tag in skb error====> Do option");
					return -1;
				}
			}
			...
			if(i%10 == 1 || i%10 == 2)
			{
				...
				{
					PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Change outer tag in skb error====> Do option");
					return -1;
				}
			}
			return 0;
		...
		default:
			PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Operation Method Error ====> Do option");
			return -1;
	}
...
int save_mac_vlan_info(struct sk_buff * skb,pon_vlan * vlan_data,original_tag otag, __u8 uprule_index)
{
	...
	if((XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_LAN) || (XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_TX_CALL_HOOK))
	{
				...
				if((mv_pair->vid_mask & ORG_INNER_VID) != 0 && mv_pair->org_inner_vid == getVID(otag.inner_tci))
				{
					if(otag.tag_num > 1)
					{
						if((mv_pair->vid_mask & ORG_OUTER_VID) != 0 && mv_pair->org_outer_vid == getVID(otag.outer_tci))
						{
							...
							PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->vlan_tag_flag,"update vid1. the upstream mac:%02x:%02x:%02x:%02x:%02x:%02x, uprule_index:%d,org_vid:%x, rs_vid:%x, i:%d\n",
								mv_pair->src_mac[0], mv_pair->src_mac[1],mv_pair->src_mac[2], mv_pair->src_mac[3],mv_pair->src_mac[4], mv_pair->src_mac[5],
								uprule_index, getVID(otag.inner_tci), getVID(XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num - 1]), i);
						}
					}
					else
					{
					...
					//if(((tv.tv_sec - mv_pair->last_time) > 1) && hwnat_clean_entry_by_dst_mac_hook)
					//	hwnat_clean_entry_by_dst_mac_hook(mv_pair->src_mac);
					PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->vlan_tag_flag,"update vid2. the upstream mac:%02x:%02x:%02x:%02x:%02x:%02x, uprule_index:%d,org_vid:%x, rs_vid:%x, i:%d\n",
						mv_pair->src_mac[0], mv_pair->src_mac[1],mv_pair->src_mac[2], mv_pair->src_mac[3],mv_pair->src_mac[4], mv_pair->src_mac[5],
						uprule_index, getVID(otag.inner_tci), getVID(XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num - 1]), i);
				}
		...
		for(i = 0; i < MAX_MAC_VID; i++)
		{
				...
				|| ((tv.tv_sec - mv_pair->last_time) > pon_vlan_all_data.mac_vlan_time))//time out rule
			{
				...
				mv_pair->uprule_index = uprule_index;
				PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->vlan_tag_flag,"record the upstream mac:%02x:%02x:%02x:%02x:%02x:%02x, uprule_index:%d,org_vid:%x, rs_vid:%x, i:%d",
					mv_pair->src_mac[0], mv_pair->src_mac[1],mv_pair->src_mac[2], mv_pair->src_mac[3],mv_pair->src_mac[4], mv_pair->src_mac[5],
					uprule_index, getVID(otag.inner_tci), getVID(XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num - 1]), i);
				...
				mv_pair->last_time = tv.tv_sec;
				if(XPON_SKB_CB(skb)->pon_tag_num <= 4)
				{
					mv_pair->vid_mask |= RULE_ENABLE;
					mv_pair->rs_outer_vid = getVID(XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num - 1]);
				}
				break;
			}
			mv_pair++;
	...
	for(i = 0; i < MAX_MAC_VID; i++)
	{
		if(mv_pair->vid_mask & RULE_ENABLE)
		{
			...
			if(memcmp(skb->data,mv_pair->src_mac,6) == 0)
			{
				...
				{
					if(XPON_SKB_CB(skb)->pon_tag_num == 1 && (mv_pair->vid_mask & ORG_INNER_VID) && !(mv_pair->vid_mask & ORG_OUTER_VID))
					{
						setVID(XPON_SKB_CB(skb)->pon_vlan_tci[0],getVID(mv_pair->org_inner_vid));
					}
					if(XPON_SKB_CB(skb)->pon_tag_num == 2 && (mv_pair->vid_mask & ORG_INNER_VID) && (mv_pair->vid_mask & ORG_OUTER_VID))
					{
						setVID(XPON_SKB_CB(skb)->pon_vlan_tci[0],getVID(mv_pair->org_inner_vid));
						setVID(XPON_SKB_CB(skb)->pon_vlan_tci[1],getVID(mv_pair->org_outer_vid));
					}
					break;
				}
			}
...
struct sk_buff* eponVlanPriRemark(struct sk_buff *skb)
{
    ...
    uc802prio = XPON_SKB_CB(skb)->epon_pbit;
...
int insert_tag_function(struct sk_buff **pskb)
{
    ...
    for(i = 0; i < XPON_SKB_CB(skb)->pon_tag_num; i++)
    {
        PONVLAN_PRINT(PONVLAN_MSG_TRACE,"In the Pon Insert Tag,vlan tag index is %d,tpid is %x,tci is %x",i,XPON_SKB_CB(skb)->pon_vlan_tpid[i],XPON_SKB_CB(skb)->pon_vlan_tci[i]);
        skb = __pon_vlan_put_tag(skb, XPON_SKB_CB(skb)->pon_vlan_tpid[i], XPON_SKB_CB(skb)->pon_vlan_tci[i]);
        if (skb == NULL)
            return -1;
    }
    
    if( XPON_SKB_CB(skb)->pon_vlan_flag & PON_CLASSIFICATION_REMARK ) 
    {
...
#if 0
    if((XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK) && pon_vlan_all_data.onu_mode == MODE_HGU)
    {
        if(DBG_Level > 1)
        {
            printk("skb info is");
...
int send_broadcast_by_multi_rules(struct sk_buff *skb)
{
    ...
    if(vlan_data == NULL)
    {
       PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"no rule for this packet,discard packet");
       return -1;
    }
    else
    {
    ...
    for(i = vlan_data->down_rule_count-1;  i >= 0; i--)
    {
        ...
        if(skb_tmp == NULL){
            PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag," skb copy error");
            return -1;
        }
        ...
        {
            kfree_skb(skb_tmp);
            PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag," store tag info error");
            continue;
        }
        memset(&otag, 0, sizeof(otag));

		otag.tag_num = XPON_SKB_CB(skb_tmp)->pon_tag_num;
		if(XPON_SKB_CB(skb_tmp)->pon_tag_num > 0)
		{
		   otag.inner_tpid = XPON_SKB_CB(skb_tmp)->pon_vlan_tpid[0];
		   otag.inner_tci = XPON_SKB_CB(skb_tmp)->pon_vlan_tci[0];
		}
		if(XPON_SKB_CB(skb_tmp)->pon_tag_num > 1)
		{
		   otag.outer_tpid= XPON_SKB_CB(skb_tmp)->pon_vlan_tpid[1];
		   otag.outer_tci = XPON_SKB_CB(skb_tmp)->pon_vlan_tci[1];
        }
        ...
        /* for broadcast frame, will never matched by mac, so only need to ignore the first matched rule to avoid the repeat frame. */
            if(have_match == 0)
            {
                have_match = 1;
                kfree_skb(skb_tmp);
                PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag," ignore matched rule. ");
                continue;
            }
            if(do_option(skb_tmp, vlan_data, DOWNSTREAM, i, otag, downMatchFlag) == -1)
            {
                kfree_skb(skb_tmp);
                PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag," do option error");
                continue;
            }
        }
        ...
        {
            PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag," insert tag error");
            continue;
        }
...
int pon_vlan_filter(struct sk_buff *skb)
{
    ...
    if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_SEND_TO_WAN)
        direction = UPSTREAM;
    else
    {
        direction = get_stream_direction(XPON_SKB_CB(skb)->pon_vlan_flag);
	...
	if(direction == UPSTREAM && XPON_SKB_CB(skb)->pon_tag_num == 0)
	{
		pon_check_untag(skb);
	}
/*according to the original vlan tag, execute ANI RX and UNI TX vlan filter, downstream*/
    ...
    if(gponVlanFilterDbgFlag >=  GPON_VLAN_FILTER_DEBUG_LEVEL_TRACE){
        if( direction == DOWNSTREAM )
            printk("\r\n pon_insert_tag->ANI_RX, UNI_TX, XPON_SKB_CB(skb)->pon_tag_num=%d",XPON_SKB_CB(skb)->pon_tag_num);
        else if( direction == UPSTREAM)
            printk("\r\n pon_insert_tag->UNI_RX, ANI_TX, XPON_SKB_CB(skb)->pon_tag_num=%d",XPON_SKB_CB(skb)->pon_tag_num);
        for(k = 0; k < XPON_SKB_CB(skb)->pon_tag_num; k++){
            printk("\r\n XPON_SKB_CB(skb)->pon_vlan_tci[%d]=0x%02x",k,XPON_SKB_CB(skb)->pon_vlan_tci[k]);
        }
    }
    if( direction == DOWNSTREAM )
    ...
    if((result == GPON_VLAN_FILTER_SUCCESS) && (discardFlag == GPON_PKT_VLAN_FILTER_DISCARD)){
		if(pon_vlan_ani_filter(skb, ponVlanFilterDirFlag) == -1){
	        if(gponVlanFilterDbgFlag >=  GPON_VLAN_FILTER_DEBUG_LEVEL_TRACE || (XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_TRACE_FLAG) ){
	            if( direction == DOWNSTREAM )
        ...
        if(gponVlanFilterDbgFlag >=  GPON_VLAN_FILTER_DEBUG_LEVEL_ERROR || (XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_TRACE_FLAG) ){
            if( direction == DOWNSTREAM )
        ...
        for(i = 1; i < pon_vlan_all_data.total_port_count; i++)
        {
            if(pon_vlan_all_data.pon_vlan_data[i].port_index == port)
            {
                if(handle_igmp_tag(&skb,pon_vlan_all_data.pon_vlan_data[i].igmp_mode,pon_vlan_all_data.pon_vlan_data[i].igmp_tci) == -1){
                    PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Igmp handle tag fail");
                    return -1;
                }
                return 1;
            }
        }
        PONVLAN_PRINT(PONVLAN_MSG_TRACE|XPON_SKB_CB(skb)->pon_vlan_flag,"no rule for this igmp packet,do nothing here");
        return 1;
    }
    else
    {
        PONVLAN_PRINT(PONVLAN_MSG_TRACE|XPON_SKB_CB(skb)->pon_vlan_flag,"Igmp add Tag function is disable,do as normal packet");
    }
    return 0;
}
int change_pkt_by_match_rule(struct sk_buff *skb, __u8 direction, pon_vlan * vlan_data, original_tag otag)
{
    ...
    {
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"Match the Rule %d,we will do the option",MatchIndex);
        
        if(do_option(skb, vlan_data, direction, MatchIndex, otag, matchFlag) == -1)
        {
            PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Do Option Error,drop packet");
            return -1;
        }
        else
        {
            if(vlan_data->mac_bind_vlan_enable == 1 && direction == UPSTREAM){
                PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"Match the Rule %d, save the upstream rule index by mac", MatchIndex);
                save_mac_vlan_info(skb,vlan_data,otag,MatchIndex);
            }
            else if(vlan_data->mac_bind_vlan_enable == 1 && direction == DOWNSTREAM){
                PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"Match the Rule %d, set the old mac enable info", MatchIndex);
    ...
    {
        PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"not match the default rule,drop packet");
        return -1;
    }
    
    PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"can not find rule");
    return 0;
}
int handle_common_pkt(struct sk_buff *skb, int *downstream_broadcast_flag)
{
    ...
    {
        PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"unknown the stream direction");
        return 0;
    }
    ...
    {
       PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"Find Vlan rule in port %d",vlan_data->port_index);
    }


    otag.tag_num = XPON_SKB_CB(skb)->pon_tag_num;
    if(XPON_SKB_CB(skb)->pon_tag_num > 0)
    {
       otag.inner_tpid = XPON_SKB_CB(skb)->pon_vlan_tpid[0];
       otag.inner_tci = XPON_SKB_CB(skb)->pon_vlan_tci[0];
    }
    if(XPON_SKB_CB(skb)->pon_tag_num > 1)
    {
       otag.outer_tpid= XPON_SKB_CB(skb)->pon_vlan_tpid[1];
       otag.outer_tci = XPON_SKB_CB(skb)->pon_vlan_tci[1];
    }
    ...
    if(direction == UPSTREAM)
    {
        PONVLAN_PRINT(PONVLAN_MSG_TRACE,"upstream print,tag num is %d",XPON_SKB_CB(skb)->pon_tag_num);
        
        if((pon_vlan_all_data.veip_enable_flag == DISABLE) && (pon_vlan_all_data.onu_mode == MODE_HGU))
        {
        	if((XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_VEIP)  // hybrid hgu port
				|| (hy_enable == 0 && (XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_TX_CALL_HOOK)))  //pure hgu
        	{
	            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"upstream print, VEIP Disable, HGU Mode, TX_CALL_HOOK");
	            return 1;
        	}
        }
        ...
        if(ret != 0)
        {
            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"upstream print, match the rule return value=%d",ret);
            return ret;
        }
        ...
        if(vlan_data->port_index != default_portIndex && vlan_data->enable_default_rule)
        {
            ...
            if(ret != 0)
            {
                PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"upstream print, match default rule return value=%d",ret);
                return ret;
            }
        }
        PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"upstream print,no rule match");
        return -1;
    }
    else if(direction == DOWNSTREAM)
    {
        PONVLAN_PRINT(PONVLAN_MSG_TRACE,"downstream print, tag num is %d",XPON_SKB_CB(skb)->pon_tag_num);
        
        if((pon_vlan_all_data.veip_enable_flag == DISABLE) && (pon_vlan_all_data.onu_mode == MODE_HGU)) 
        {
        	if((hy_enable == 1 && !(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_SFU_WAN))  // hybrid hgu port
				|| (hy_enable == 0 && (XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK)))  //pure hgu
        	{
	            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"downstream print, VEIP Disable, HGU Mode, PON_VLAN_RX_CALL_HOOK");
	            return 1;
        	}
        }

        if(hy_enable == 1 && XPON_SKB_CB(skb)->pon_tag_num == 0 && !(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_SFU_WAN))
        {
            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"downstream print, Hybrid VEIP untag pkt pass");
            return 1;
        }
        
        if(vlan_data->down_stream_mode == TRANSPARENT_MODE)
        {
            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"downstream print,in transparent mode");
            return 1;
        }
        
        if(vlan_data->down_stream_mode == STRIPPED_MODE)
        {
            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"downstream print,in stripped mode");
            if(XPON_SKB_CB(skb)->pon_tag_num > 0)
                XPON_SKB_CB(skb)->pon_tag_num--;
            return 1;
        }
        ...
        if(vlan_data->mac_bind_vlan_enable == 1)
        {
            ...
            if(iMatchIndex > -1 && iMatchIndex < vlan_data->down_rule_count)
            {
                ...
                else
                {
                    ...
                    if(ret != 0)
                    {
                        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"downstream print, match the rule return value=%d",ret);
                        return ret;
                    }
                }
            }
        ...
        if(TCSUPPORT_CHS_VAL && vlan_data->down_rule_count > 0){
            PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"TCSUPPORT_CHS no specify rule match the packet,drop packet");
            return -1;
        }
		...
		{/* 247测试用例6.4 */
	        broadcast_dst_mac = skb->data;;
	        if(checkBroadcastPkt(broadcast_dst_mac) == 1){
	            PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"TCSUPPORT_BBF_247_VAL, packet is broadcast,drop packet");
	            return -1;
	        }
        }
		...
		{
			PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Downstream no rule match the packet,drop packet\n");
        	return -1;
		}  
    }
    return 0;
...
{
	char tmp[80];
	char ingress_name[IFNAMSIZ] = "<none>";
	char *p = skb->data;
	char *t = tmp;
	int i, n = 0;
	...
	xpon_skb_copy_ingress_name(skb, ingress_name, sizeof(ingress_name));
	printk("skb dev %s, orig dev %s\n", skb->dev->name, ingress_name);
...
int is_to_or_from_br0(struct sk_buff *skb)
{
	...
	{
		if((memcmp(dest, device->dev_addr, 6) == 0) && (XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_VEIP)){
			PONVLAN_PRINT(PONVLAN_MSG_WARNING|PON_PKT_TRACE_FLAG,"hgu port to br0");
			return 1;
		}
		else if((memcmp(dest, bcast, 6) == 0)  &&  (XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_VEIP)){
			PONVLAN_PRINT(PONVLAN_MSG_WARNING|PON_PKT_TRACE_FLAG,"hgu port to broadcast");
			return 1;
		}
		else if( (memcmp(src, device->dev_addr, 6) == 0)  &&  !(XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK)){
...
int pon_insert_tag(struct sk_buff **pskb)
{
    ...
    skb = *pskb;
    out_dev = skb->dev;
    PONVLAN_PRINT(PONVLAN_MSG_TRACE,"Skb flag is %x, out_dev is %s  hy_enable=%d ",XPON_SKB_CB(skb)->pon_vlan_flag,out_dev->name,hy_enable);

    XPON_SKB_CB(skb)->pon_vlan_flag |= (is_trace_skb(skb)?PON_PKT_TRACE_FLAG:0);

    if(pon_vlan_all_data.vlan_enable_flag == 0) {//disable vlan function
        if (pon_vlan_all_data.onu_mode == MODE_HGU){
            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag," vlan disable and onu type is HGU");
            return 0; /* modify by sun.zhu */
        }else{
            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"vlan disable and onu type is SFU ");
            goto Insert_Tag;
        }
    }

    
    if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_LEAVE_PKT_DEAL)
    {
        return 0;
    }
    ...
    if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_SEND_TO_WAN)//gpon sent to wan,do vlan filter here
    {
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"LAN->WAN vlan filter goto");
        goto VLAN_Filter;
    }
    
    if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_INSERT_FLAG)
    {
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"frame tag has been handle,just return 0");
        return 0;
    }
    

	if((DBG_Level & PONVLAN_DROP_DUMP_UP) && 
		(XPON_SKB_CB(skb)->pon_vlan_flag & (PON_VLAN_TX_CALL_HOOK | PON_PKT_FROM_CPE | PON_PKT_FROM_LAN | PON_PKT_VOIP_TX | PON_PKT_FROM_HYBRID_VEIP | PON_PKT_FROM_HYBRID_PPTP))){
		printk("[%s][%d] flag:%x,mark:%x\n",__FUNCTION__, __LINE__, XPON_SKB_CB(skb)->pon_vlan_flag, skb->mark);
		dump_skb(skb);
	}
	
	if((DBG_Level & PONVLAN_DROP_DUMP_DOWN) && 
		(XPON_SKB_CB(skb)->pon_vlan_flag & (PON_VLAN_RX_CALL_HOOK | PON_PKT_FROM_WAN | PON_PKT_VOIP_RX))){
		printk("[%s][%d] flag:%x,mark:%x\n",__FUNCTION__, __LINE__, XPON_SKB_CB(skb)->pon_vlan_flag, skb->mark);
		dump_skb(skb);
	}

    if(is_valid_tx_rx_pkt(XPON_SKB_CB(skb)->pon_vlan_flag, out_dev)){
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"valid pkt");
		if(hy_enable == 1  && !memcmp(out_dev->name,"pon",3) && XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK)
		{
        ...
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"invalid tx rx pkt ");
        return 0;
    }

    if(is_cpe2lan_pkt(XPON_SKB_CB(skb)->pon_vlan_flag, out_dev))// cpe2lan direction.do nothing
    {
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"pkt lan2lan goto insert tag");
        goto Insert_Tag;
    }

    /* ipoa, no mac header, don't should insert vlan tag,it shouldn't be here anyway*/
    if (skb->data == skb_network_header(skb)) { 
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"ipoa pkt, not handle here");
        return 0;
    }
    ...
    *pskb = skb;
    ...
    if(((XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK) && pon_vlan_all_data.onu_mode == MODE_HGU && !memcmp(out_dev->name,"pon",3))  //in pon dev,downstream
		|| (hy_enable==1 && !memcmp(out_dev->name,"eth",3) && !(XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK))) //hybrid upstream, not pon dev
    ...
    {
        PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Store tag info error when bridge mode.return -1. ====> Pon insert tag");
        goto Fail;
    }
    if(TCSUPPORT_PON_VLAN_FILTER_VAL && (ret = pon_vlan_filter(skb)) == -1){
        PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"pon vlan filter fail");
        goto Fail;
    }

    if(hy_enable ==1  && !memcmp(out_dev->name,"pon",3) && XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK) //hybrid downstream in pon dev
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"hybrid downstream in pon dev only do filter pon_vlan_flag=%x\n",XPON_SKB_CB(skb)->pon_vlan_flag);
		goto Insert_Tag;
    }
    ...
    if(ret == -1){
        PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"handle igmp pkt fail");
        goto Fail;
    }else if(ret == 1){
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"handle igmp pkt success");
        goto Insert_Tag;
    }

	lan2lan = is_lan2lan_pkt(XPON_SKB_CB(skb)->pon_vlan_flag, out_dev);
	if(lan2lan)//lan2lan should do upstream vlan operations according the source port.
	{
		rcu_read_lock();
		ingress_rcu_locked = true;
		skb->dev = xpon_skb_ingress_dev_rcu(skb);
	}

	ret = handle_common_pkt(skb, &ds_broadcast_flag);
	if (ingress_rcu_locked) {
		skb->dev = out_dev;
		rcu_read_unlock();
	}

    if(ret == -1){
        PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"handle common pkt fail");
        goto Fail;
    }else if(ret == 1){
		if(lan2lan)//lan2lan should do downstream vlan operation according the dest port.
        {
            XPON_SKB_CB(skb)->pon_vlan_flag = (XPON_SKB_CB(skb)->pon_vlan_flag & (~PON_PKT_FROM_LAN));
            XPON_SKB_CB(skb)->pon_vlan_flag |= PON_PKT_FROM_WAN;
            skb->dev = out_dev;
            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"pkt lan2lan double treat.");
            ret = handle_common_pkt(skb, &ds_broadcast_flag);

            if(ret == -1){
                PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"handle common lan2lan pkt fail");
                goto Fail;
            }else if(ret == 1){
                PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"handle common lan2lan pkt success");
                goto Insert_Tag;
            }

        }
        else{
            PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"handle common pkt success");
            goto Insert_Tag;
        }
    }
...
VLAN_Filter:
    ...
    if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_SEND_TO_WAN){
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"pon vlan filter success LAN->WAN");
        if(skb_tmp != NULL){
            kfree_skb(skb_tmp);
    		skb_tmp = NULL;
        }
...
Insert_Tag:
    ...
    for(i = 0; i < XPON_SKB_CB(skb)->pon_tag_num; i++)
    {
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"In the Pon Insert Tag,vlan tag index is %d,tpid is %x,tci is %x",i,XPON_SKB_CB(skb)->pon_vlan_tpid[i],XPON_SKB_CB(skb)->pon_vlan_tci[i]);
        skb = __pon_vlan_put_tag(skb, XPON_SKB_CB(skb)->pon_vlan_tpid[i], XPON_SKB_CB(skb)->pon_vlan_tci[i]);
        if (skb)
            skb->dev = out_dev;
        else{
    ...
    if(((XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK) && pon_vlan_all_data.onu_mode == MODE_HGU && !memcmp(out_dev->name,"pon",3)) //hgu, downstream
		|| (hy_enable==1 && !memcmp(out_dev->name,"eth",3) && !(XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_RX_CALL_HOOK))) // hybrid, not pon dev
	
    {
        ...
        skb->data += 12;
        skb->protocol = *(__u16 *)(skb->data);
        PONVLAN_PRINT(PONVLAN_MSG_WARNING|XPON_SKB_CB(skb)->pon_vlan_flag,"In the Pon Insert Tag,HGU ds print,protocol is  %x,skb data is %02x",skb->protocol,skb->data[0]);
        skb->data += 2;
    }
    
	if((DBG_Level & PONVLAN_DROP_DUMP_UP) && 
		(XPON_SKB_CB(skb)->pon_vlan_flag & (PON_VLAN_TX_CALL_HOOK | PON_PKT_FROM_CPE | PON_PKT_FROM_LAN | PON_PKT_VOIP_TX))){
		printk("[%s][%d] flag:%x,mark:%x\n",__FUNCTION__, __LINE__, XPON_SKB_CB(skb)->pon_vlan_flag, skb->mark);
		dump_skb(skb);
	}
...
Fail:
    ...
    if(skb_tmp != NULL)
        kfree_skb(skb_tmp);
    PONVLAN_PRINT(PONVLAN_MSG_ERR|XPON_SKB_CB(skb)->pon_vlan_flag,"Insert Tag Error,we will discard packet");
    return -1;
}
...
/*
	check the skb data, if XPON_SKB_CB(skb)->pon_tag_num is 0.
*/
int pon_check_untag(struct sk_buff *skb)
{
	...
	{
		XPON_SKB_CB(skb)->pon_vlan_tci[j] = ntohs(vhethdr->h_vlan_TCI);
		XPON_SKB_CB(skb)->pon_vlan_tpid[j] = vheth_tpid;
		XPON_SKB_CB(skb)->pon_tag_num++;
		j++;
		...
		while(((vh_tpid == 0x8100)||(vh_tpid == 0x88a8)||(vh_tpid == 0x9100))&&(j < 4)){
			XPON_SKB_CB(skb)->pon_vlan_tci[j] = ntohs(*(u16 *)(vhdr+1));
			XPON_SKB_CB(skb)->pon_vlan_tpid[j] = vh_tpid;
			XPON_SKB_CB(skb)->pon_tag_num++;
			j++;
			vhdr++;
		}
...
int pon_store_vlan_tag(struct sk_buff *skb, struct net_device *orig_dev)
{
	...
	if((orig_dev != NULL) && ((orig_dev->name[0] == 'b') || (orig_dev->name[0] == 'n')))
	{
		...
		XPON_SKB_CB(skb)->pon_vlan_tci[0] = vhdr->h_vlan_TCI;
		XPON_SKB_CB(skb)->pon_vlan_tpid[0] = *(u16 *)tmp;
		XPON_SKB_CB(skb)->pon_tag_num++;
		/* Take off the VLAN header (4 bytes currently) */
		skb_pull_rcsum(skb, VLAN_HLEN);
		skb->dev = orig_dev;
...
int pon_check_user_group(struct sk_buff *skb)
{
	...
	if(!(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_LAN) || (XPON_SKB_CB(skb)->pon_vlan_flag & PON_USER_GROUP_FLAG))//only check lan port.
		return 1;

	if(out_dev->name[0] == 'e' || out_dev->name[0] == 'r' || out_dev->name[0] == 'u')
		XPON_SKB_CB(skb)->pon_vlan_flag |= PON_USER_GROUP_FLAG;
	else
		return 1;
...
int pon_pcp_decode(struct sk_buff **pskb)
{
	...
	if(XPON_SKB_CB(skb)->pon_tag_num != 0)
	{
		pcp_decode(&(XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num - 1]),pon_vlan_all_data.pcp_mode);
...
#ifdef PONVLAN_IOC_DOWNSTREAM_FORWARD_OPT
...
#else
	vlan_data = timer_container_of(vlan_data, data, resort_timer);
#endif 
	if(NULL == vlan_data){
		return;
	}
...
*/
#ifdef PONVLAN_IOC_EPON_RULE_OPT
...
#ifdef PONVLAN_IOC_ENABLE_TRACE_DROP
...
static int vlan_data_clean(pon_vlan_all * data)
{
	...
	{
		timer_delete_sync(&vlan_data->resort_timer);
...
#endif
{
...
#ifdef PONVLAN_IOC_VLAN_MULT_FILTER_ENABLE
	...
	switch(cmd)
	{
		...
		case PONVLAN_IOC_EPON_RULE_OPT:
			...
			break;
#endif
#ifdef PONVLAN_IOC_XPON_MODE_OPT
		...
		case PONVLAN_IOC_VLAN_MULT_FILTER_ENABLE:
			...
			break;
#endif
#ifdef PONVLAN_IOC_UNI_FILTER_SWITCH_OPT
		...
		case PONVLAN_IOC_GET_HYBRID_PORT_MASK:
			...
			break;
#endif
#ifdef PONVLAN_IOC_SET_DS_BCAST_1_TO_N_OPT
...
EXPORT_SYMBOL(pon_vlan_get_down_vlan_opt);
...
int pon_vlan_is_ds_1_to_N(struct sk_buff **pskb, int *count)
{
	...
	if(XPON_SKB_CB(skb)->pon_tag_num > 0)
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR,"pon_tag_num 0, tci=%x",XPON_SKB_CB(skb)->pon_vlan_tci[0]);
	}
	if(XPON_SKB_CB(skb)->pon_tag_num > 1)
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR,"pon_tag_num 1, tci=%x",XPON_SKB_CB(skb)->pon_vlan_tci[1]);
	}
...
restore_skb:
	for( i = 0; i < XPON_SKB_CB(skb)->pon_tag_num; i++ )
	{
		PONVLAN_PRINT(PONVLAN_MSG_ERR, "vlan tag index is %d, tpid is %x, tci is %x", i, XPON_SKB_CB(skb)->pon_vlan_tpid[i], XPON_SKB_CB(skb)->pon_vlan_tci[i]);
		skb = __pon_vlan_put_tag(skb, XPON_SKB_CB(skb)->pon_vlan_tpid[i], XPON_SKB_CB(skb)->pon_vlan_tci[i]);
		if( skb == NULL )
		{
			ret = 0;
