...
int matchVlanFilterRule(struct sk_buff *skb, __u8 ponVlanFilterDirFlag, __u8 * discardFlag){
	...
	if(gponVlanFilterDbgFlag >=  GPON_VLAN_FILTER_DEBUG_LEVEL_TRACE)
		printk("\r\n matchVlanFilterRule-->XPON_SKB_CB(skb)->pon_tag_num=%d",XPON_SKB_CB(skb)->pon_tag_num);
	if(XPON_SKB_CB(skb)->pon_tag_num == GPON_VLAN_FILTR_VLAN_TAG_NUM_0){
		type = GPON_VLAN_FILTR_TYPE_UNTAGGED;
	}else{
		type = GPON_VLAN_FILTR_TYPE_TAGGED;
		vlan_tag = XPON_SKB_CB(skb)->pon_vlan_tci[XPON_SKB_CB(skb)->pon_tag_num-1];
	}
	if(gponVlanFilterDbgFlag >=  GPON_VLAN_FILTER_DEBUG_LEVEL_TRACE)
	...
	if((ponVlanFilterDirFlag & GPON_VLAN_FILTER_HANDLE_ANI_RX_VLAN_TAG)
		...
		*****************************************************************************************************/
            ...
            if(0 == (XPON_SKB_CB(skb)->pon_vlan_flag & PON_MULTICAST_ANI_FILTER_FLAG) && 
		...
		*****************************************************************************************************/
			...
			if(TCSUPPORT_PON_IP_HOST_VAL && (XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_VOIP_RX)){
		...
		*****************************************************************************************************/
			...
			if(!(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_SEND_TO_WAN)){
				...
				if(gponVlanFilterDbgFlag >= GPON_VLAN_FILTER_DEBUG_LEVEL_ERROR) {
					...
					xpon_skb_copy_ingress_name(skb, ingress_name,
								   sizeof(ingress_name));
					printk("\r\n upstream UNI:XPON_SKB_CB(skb)->pon_vlan_flag == %d ingress_dev == %s",
						XPON_SKB_CB(skb)->pon_vlan_flag, ingress_name);
				}
