...
#define pon_mac_filter_isdigit(x)	((x)>='0'&&(x)<='9')
...
static inline int atoi(char *s)
{
	...
	if(s == NULL)
	{
        ...
        while (pon_mac_filter_isdigit(*s)) {
                i = i*10 + *(s++) - '0';
        }
        return i;
}
...
static inline pon_mac_filter * get_filter_opt_data(struct sk_buff *skb)
{
	...
	if(XPON_SKB_CB(skb)->pon_mac_filter_flag & PKT_SEND_TO_WAN)
	{
		if(DBG_Level > 1)
			printk("\r\npacket send to wan,virtual if is %d",XPON_SKB_CB(skb)->v_if);
		tmp = &pon_mac_filter_all_data.upstream_data[XPON_SKB_CB(skb)->v_if];
	}
	else if(XPON_SKB_CB(skb)->pon_mac_filter_flag & PON_MAC_FILTER_RX_CALL_HOOK)
	{
	...
	else if(XPON_SKB_CB(skb)->pon_mac_filter_flag & PON_MAC_FILTER_TX_CALL_HOOK)
	{
		if(DBG_Level > 1)
			printk("\r\nSFU mode,get lan port");
		if(dev->name[0] == 'e')
...
int check_rule(struct sk_buff *skb,pon_mac_filter * data)
{
	...
	if (PON_MAC_FILTER_RX_CALL_HOOK ==(XPON_SKB_CB(skb)->pon_mac_filter_flag & PON_MAC_FILTER_RX_CALL_HOOK)
	...
	if((data->unicast_rule_counter != 0) && (!is_rx))/*we check unicast rule first*/
	{
		for(i = 0; i < data->unicast_rule_counter; i++)
		{
			...
			if(rule->ethertype != 0)
			{
				...
				{
					mac = skb->data + 12 + 4 * XPON_SKB_CB(skb)->pon_tag_num;
			...
			if(XPON_SKB_CB(skb)->pon_mac_filter_flag & PON_MAC_FILTER_RX_CALL_HOOK)
				mac -= 14;
	...
	if(data->multicast_rule_counter != 0)//now we check multicast rule
	{
			...
			&& (skb->dev->name[0] == 'e'))
		{
			...
			if (TCSUPPORT_PON_VLAN_VAL)
			{
				mac = temp + 12 + 4 * XPON_SKB_CB(skb)->pon_tag_num;
				ethertype = (*mac << 8) + *(mac + 1);
			}
			else
			{
...
int pon_check_mac(struct sk_buff *skb)
{
	...
	if(XPON_SKB_CB(skb)->pon_mac_filter_flag & PKT_FILTER_FLAG)
		return 0;

	if((((XPON_SKB_CB(skb)->pon_mac_filter_flag & PON_MAC_FILTER_RX_CALL_HOOK) &&  (strcmp(out_dev->name,"pon") == 0))
		|| (XPON_SKB_CB(skb)->pon_mac_filter_flag & PON_MAC_FILTER_TX_CALL_HOOK)/*HGU mode,check tx/rx hook */ )
		|| (XPON_SKB_CB(skb)->pon_mac_filter_flag & PKT_SEND_TO_WAN))
		XPON_SKB_CB(skb)->pon_mac_filter_flag |= PKT_FILTER_FLAG;
	else
		return 0;
