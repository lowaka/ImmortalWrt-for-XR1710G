...
int get_if_mask_by_gem_port(struct sk_buff *skb, u32 * ifmask)
{
	...
	while(counter != 0)
	{
		...
		//printk("\r\nskb gemport is %d,rule gemport is %d",XPON_SKB_CB(skb)->gem_port,gpon_downstream_mapping_data.Rule[pos + offset].gem_port_num);
		if(XPON_SKB_CB(skb)->gem_port == gpon_downstream_mapping_data.rule[pos + offset].gem_port_num)
		{
			*ifmask = gpon_downstream_mapping_data.rule[pos + offset].if_mask;
			...
			{
				setDownQueueID(XPON_SKB_CB(skb)->pon_mark, gpon_downstream_mapping_data.rule[pos + offset].queue);
				setDownQueueEnable(XPON_SKB_CB(skb)->pon_mark, TSENABLE);
			}
			else
			{
			    setDownQueueID(XPON_SKB_CB(skb)->pon_mark, gpon_downstream_mapping_data.rule[pos + offset].queue);
				setDownQueueEnable(XPON_SKB_CB(skb)->pon_mark, TSDISABLE);
			}
			if (gpon_downstream_mapping_data.rule[pos + offset].ds_trtcm_enable == TRAFFIC_SHAPING_ENABLE)
			{
				setDownTrtcmID(XPON_SKB_CB(skb)->pon_mark, gpon_downstream_mapping_data.rule[pos + offset].trtcmId);			
				setDownTrtcmEnable(XPON_SKB_CB(skb)->pon_mark, TSENABLE);	
			}
			else
			{
			    setDownTrtcmID(XPON_SKB_CB(skb)->pon_mark, gpon_downstream_mapping_data.rule[pos + offset].trtcmId);			
				setDownTrtcmEnable(XPON_SKB_CB(skb)->pon_mark, TSDISABLE);
			}
			return 0;
		}
		else if(XPON_SKB_CB(skb)->gem_port > gpon_downstream_mapping_data.rule[pos + offset].gem_port_num)
		{
			if(counter == 1)
				return 0;
...
int gpon_ds_queue_mapping(struct sk_buff *skb, QDMA_TxQosScheduler_T *txQos)
{
	...
	for(i = 0; i < gpon_downstream_mapping_data.rule_counter; i++)
	{
		if(XPON_SKB_CB(skb)->gem_port == gpon_downstream_mapping_data.rule[i].gem_port_num)
		{
			for (j = 0; j < 4; j++)
			{		
			...
			GPONMAP_PRINT(GPONMAP_MSG_TRACE, "[%s] skb gemport is %d,rule gemport is %d,  skb->mark is 0x%x\n\n",__FUNCTION__,
				XPON_SKB_CB(skb)->gem_port,gpon_downstream_mapping_data.rule[i].gem_port_num,skb->mark);
