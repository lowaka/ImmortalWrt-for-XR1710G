...
int mark2port(struct sk_buff *skb)
{
	...
	mark2 = GET_XFI_LAN_ITF_MARK(XPON_SKB_CB(skb)->mark2);
        ...
        for( i = 0; i < MAX_VLAN_UPSTRM_POLICER_NUM; ++ i ){
                ...
                if( ethID == upStrmPolicerTable[i].ethID && \
                    vlan_id == upStrmPolicerTable[i].vid ){
                        setTse(XPON_SKB_CB(skb)->pon_mark, TSENABLE);
                        setTsID(XPON_SKB_CB(skb)->pon_mark,
                                upStrmPolicerTable[i].chanID);
                        return 0;
                }
        }
...
not_care:
	if (skb) {
		setTse(XPON_SKB_CB(skb)->pon_mark, TSDISABLE);
		setTsID(XPON_SKB_CB(skb)->pon_mark, 0);
	}
        return 0;
}
...
int gpon_ds_uni_mapping(struct sk_buff *skb)
{
    ...
    pktInfo.gemPort = XPON_SKB_CB(skb)->gem_port;
	...
	//if(cnt ++  % 50 == 0)
	GPONMAP_PRINT(GPONMAP_MSG_WARN, "%s gem=%d,userPort=%d,skb->mark=0x%x\n",__FUNCTION__,XPON_SKB_CB(skb)->gem_port,gemport_rule_p->userPort,skb->mark);
...
int gpon_mapping(struct sk_buff *skb)
{
	...
	else
	{
		...
		if(onu_type != 1 || pktInfo.userPort == GPON_MAP_IPHOST_VOICE_PORT){ //hgu or iphost vocie port
			if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_PPTP){
...
mapping:
		...
		if (gemport_rule_p != NULL)
		{
			XPON_SKB_CB(skb)->gem_port = gemport_rule_p->gemPort;
			queue_rule_p = findQueueMappingRule(XPON_SKB_CB(skb)->gem_port);
			if (queue_rule_p != NULL)
			{
				...
				if (queue_rule_p->mode & (1<<MODE_OFFSET_TRAFFIC_SHAPING))
				{
					/* traffic shaping id*/
					setTse(XPON_SKB_CB(skb)->pon_mark, TSENABLE);
					setTsID(XPON_SKB_CB(skb)->pon_mark, ((queue_rule_p->queue & 0xF8) >> QUEUE_TRAFFIC_SHAPING_OFFSET));
				}
				else
				{
					setTse(XPON_SKB_CB(skb)->pon_mark, TSDISABLE);
			...
			GPONMAP_PRINT(GPONMAP_MSG_TRACE, "matched, gemPort = %x, queue = %x,tse = %x, tsChannelId = %x\n", 
											XPON_SKB_CB(skb)->gem_port,getQueueID(skb->mark),getTse(XPON_SKB_CB(skb)->pon_mark), getTsID(XPON_SKB_CB(skb)->pon_mark));
		}
		else
		{	
			goto free_skb;
...
int gpon_downstream_mapping_option(struct sk_buff *skb)
{
	...
	if(!(XPON_SKB_CB(skb)->pon_mark & DS_PKT_FORM_WAN))
		return 0;
	...
	GPONMAP_PRINT(GPONMAP_MSG_TRACE, "\r\nskb gemport is %d,dev name is %s",XPON_SKB_CB(skb)->gem_port,out_dev->name);
	if(get_if_mask_by_gem_port(skb,&ifmask) == -1)
		return -1;

    GPONMAP_PRINT(GPONMAP_MSG_TRACE, "\r\n skb if_mask is %x ",ifmask);
    GPONMAP_PRINT(GPONMAP_MSG_ERR, "get_if_mask_by_gem_port success gem %d skb mark is %x \n",XPON_SKB_CB(skb)->gem_port, skb->mark);

	if(getDownQueueEnable(XPON_SKB_CB(skb)->pon_mark))
	{	
	...
	if((ifmask & tmp) != 0)
	{
		...
		*/
		...
		{
			XPON_SKB_CB(skb)->pon_mark &= ~DS_PKT_MAPPING_MARK;
			XPON_SKB_CB(skb)->pon_mark |= DS_PKT_MAPPING_TO_ONE;
		}
		else
		{
			XPON_SKB_CB(skb)->pon_mark &= ~DS_PKT_MAPPING_MARK;
			XPON_SKB_CB(skb)->pon_mark |= DS_PKT_MAPPING_TO_MULTI;
		}
	}
	}
...
int gpon_ds_mapping_assign_queue(struct sk_buff **pskb)
{
	...
	if(getDownTrtcmEnable(XPON_SKB_CB(skb)->pon_mark) || (XPON_SKB_CB(skb)->pon_mark & DS_PKT_MAPPING_MARK))
    {
	    /* skb_unshare */
	...
	if(getDownTrtcmEnable(XPON_SKB_CB(skb)->pon_mark))
	{
		stag = (u16*)(skb->data + 12);
		*stag &= htons(0x03ff);//clean FUP and UPRI and DVP and DRM	
		*stag |= htons((getDownTrtcmID(XPON_SKB_CB(skb)->pon_mark) << 10));//set trtcm id
		
		GPONMAP_PRINT(GPONMAP_MSG_TRACE, "\r\nassign trtcm finished,trtcm is %d,stag is 0x%x",getDownTrtcmID(XPON_SKB_CB(skb)->pon_mark), htons(*stag));	
	}
	else if(XPON_SKB_CB(skb)->pon_mark & DS_PKT_MAPPING_MARK)//flag not 0 mean this packet need mapping to queue
	{        
		stag = (u16*)(skb->data + 12);
		*stag &= htons(0x0fff);//clean FUP and UPRI
		*stag |= htons(0x8000);//set FUP = 1,force PPE user priority
		*stag |= htons((getDownQueueID(XPON_SKB_CB(skb)->pon_mark) << 12));//set queue index
		GPONMAP_PRINT(GPONMAP_MSG_TRACE, "\r\nassign queue finished,queue is %d,stag is 0x%x",getDownQueueID(XPON_SKB_CB(skb)->pon_mark), htons(*stag));
	}
	else
	    return 0;
...
int epon_sfu_clsfy(struct sk_buff *skb,  int port)
{
	...
	if (!getMappingEnable(port)){
		...
		//XPON_SKB_CB(skb)->v_if = 0;
		//setQueueID(skb->mark, pktInfo.pbit);
		//setNewPriority(XPON_SKB_CB(skb)->gem_port, 0xFF);// default don't remark pbit
		//return 1;
...
findMappingRule:	
	...
	if (pRule != NULL){
		queue = pRule->queue;
		pbit = pRule->pbit;
		XPON_SKB_CB(skb)->pon_vlan_flag |= PON_CLASSIFICATION_REMARK;
...
findLLIDQueueRule:	
	XPON_SKB_CB(skb)->epon_queue = queue;
	XPON_SKB_CB(skb)->epon_pbit = pbit;
...
int epon_mapping(struct sk_buff *skb)
{
	...
	if (isSfu){ 
		queue = XPON_SKB_CB(skb)->epon_queue;
		pbit = XPON_SKB_CB(skb)->epon_pbit;
	}else{
		queue = (skb->mark & QOS_FILTER_MARK) >> 4;
	...
	if (pMap != NULL){
		...
		if(uni_1g){
			if(uni_llid_map[uni_1g-1].enable){
				XPON_SKB_CB(skb)->v_if = uni_llid_map[uni_1g-1].default_llid;
				QOS_PRINT(DBG_INFO, "map enabled, 1G :XPON_SKB_CB(skb)->v_if = %d\n", XPON_SKB_CB(skb)->v_if);
			}
			else{
				XPON_SKB_CB(skb)->v_if = 0;
				QOS_PRINT(DBG_INFO, "map disabled 1G :XPON_SKB_CB(skb)->v_if = %d\n", XPON_SKB_CB(skb)->v_if);
			}
		}
		else
		{
			if(uni_llid_map[uni_10g-1].enable){
				XPON_SKB_CB(skb)->v_if = uni_llid_map[uni_10g-1].default_llid;
				QOS_PRINT(DBG_INFO, "map enabled, 10G :XPON_SKB_CB(skb)->v_if = %d\n", XPON_SKB_CB(skb)->v_if);
			}
			else{
				XPON_SKB_CB(skb)->v_if = 0;
				QOS_PRINT(DBG_INFO, "map disabled 10G :XPON_SKB_CB(skb)->v_if = %d\n", XPON_SKB_CB(skb)->v_if);
			}
		}
	...
	if(pMap != NULL)
	{
	    ...
	    {
	        setTse(XPON_SKB_CB(skb)->pon_mark, TSENABLE);
		    setTsID(XPON_SKB_CB(skb)->pon_mark, queue);
...
#endif
{
	...
	switch(cmd)
	{
		...
		case QUEUE_MAPPING_GET_ENTRY:
			...
			break;
		#endif
		#ifdef QUEUE_MAPPING_RECFG_ENTRY
...
#if defined(TCSUPPORT_GPON_MAPPING) && LINUX_VERSION_CODE < KERNEL_VERSION(5,6,0)
