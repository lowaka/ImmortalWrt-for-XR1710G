...
#else
	xPON_DynPreview_t* entry = container_of(arg, xPON_DynPreview_t, rep_interval_timer);
...
int xpon_ani_incoming_handle(struct sk_buff* skb)
{
	...
	{
		XPON_SKB_CB(skb)->is_unknown_mul = 1;
#ifdef TCSUPPORT_RA_HWNAT
...
static int frame_is_legal_multicast_pkt(struct sk_buff* skb, int direct) 
{
    ...
    if(MULTICAST_UP_STREAM == direct)
    {
        ...
        if(0 == (XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_LAN))
        {
    ...
    else
    {
        ...
        if(0 == (XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_WAN))
        {
