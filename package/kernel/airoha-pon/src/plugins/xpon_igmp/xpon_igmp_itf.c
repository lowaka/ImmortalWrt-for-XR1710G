...
static int xpon_igmp_major = GLOBAL_IGMP_MAJOR;
...
bool xpon_down_vlan_general_query_access(struct sk_buff* skb,int port_id)
{
	...
	XPON_SKB_CB(skb)->vlan_tags[0] = skb_vid;
	XPON_SKB_CB(skb)->vlan_tag_flag |= VLAN_PACKET;
...
int xpon_store_upstream_igmp_vlan_tci(struct sk_buff* skb)
{
	...
	if(XPON_SKB_CB(skb)->xpon_igmp_flag&XPON_IGMP_UPSTREAM_RESTORE)
		return 0;
	...
	XPON_SKB_CB(skb)->xpon_igmp_flag |= XPON_IGMP_IS_MULTICAST;
	XPON_SKB_CB(skb)->xpon_igmp_flag |= XPON_IGMP_UPSTREAM_RESTORE;
	return 1;
}
...
int xpon_upstream_vlan_recovery_by_dynlist(struct sk_buff* skb)
{
	...
	if(!(XPON_SKB_CB(skb)->xpon_igmp_flag & XPON_IGMP_IS_MULTICAST))
			return 0;

	if(XPON_SKB_CB(skb)->xpon_igmp_flag&XPON_IGMP_UPSTREAM_RECOVERY)
			return 0;
	...
	port_id = ENCT_HOOK_XPON_ETH_MAP_DEV_NAME_TO_PORT(ingress_name);
	if(-1 == port_id )
		return -1;
	...
	{		
		...
		if(-1 == recovery_vid)
		{
			XPON_SKB_CB(skb)->xpon_igmp_flag |= XPON_IGMP_UPSTREAM_RECOVERY;
			return 0;
		}
	}
	else
...
int xpon_replace_general_query_vlan(struct sk_buff** pskb)
{
	...
	{
		if(XPON_SKB_CB(skb)->vlan_tag_flag & VLAN_PACKET)
			skb_vid = XPON_SKB_CB(skb)->vlan_tags[0];
		else
			return 0;
	}
...
int xpon_down_multicast_vlan_handle(struct sk_buff** pskb)
{
	...
	if(XPON_SKB_CB(skb)->xpon_igmp_flag&XPON_IGMP_DOWNSTREAM_VLAN_HANDLE)
		return 0;
	...
	if(xpon_is_general_query(skb))
	{
		...
		XPON_SKB_CB(skb)->xpon_igmp_flag |= XPON_IGMP_DOWNSTREAM_VLAN_HANDLE;
		return 0;
	}
...
void xpon_igmp_fini(void)
{
	...
	unregister_chrdev(xpon_igmp_major, GLOBAL_IGMP_IOC_NAME);
	xpon_uninit_igmp();
	return;
}
