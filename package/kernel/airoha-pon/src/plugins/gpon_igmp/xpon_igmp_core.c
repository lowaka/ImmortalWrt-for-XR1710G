...
void xpon_igmp_store_src_vlan_info(struct sk_buff* skb)
{
	...
	if (eth_type == 0x8100 || eth_type == 0x88a8)
	{
	    /* SFU mode, XPON_SKB_CB(skb)->vlan_tags is not used in other ways */
		XPON_SKB_CB(skb)->vlan_tags[0] = eth_type;
		XPON_SKB_CB(skb)->vlan_tags[1] = ntohs(*(unsigned short int*)(buff+14));	
	}
...
int xpon_get_igmp_port_by_original_dev(struct sk_buff* skb)
{
	...
	if(!xpon_skb_copy_ingress_name(skb, ingress_name,
					   sizeof(ingress_name))){
		port_id = 0;
	}
	/*eth0.x*/
	else if('e' == ingress_name[0])
		port_id = ingress_name[5] - '0';
	/*ra0*/
	else if('r' == ingress_name[0])
		port_id = 5;
	else
		port_id = 0;
...
int xpon_forward_to_port(struct sk_buff* skb,int port)
{
    ...
    XPON_SKB_CB(skb)->pon_vlan_flag |= PON_MULTICAST_ANI_FILTER_FLAG;
...
#else
	xPON_FwdEntry_t* entry = container_of(arg, xPON_FwdEntry_t, leave_ageing_timer);
#endif
...
static void xpon_preview_timer_timeout(TIMER_FUN_PAAM arg)
{
	...
	xPON_FwdEntry_t* entry = (xPON_FwdEntry_t* )arg;
#else
	xPON_FwdEntry_t* entry = container_of(arg, xPON_FwdEntry_t, preview_timer);
#endif
...
int xpon_leave_pkt_handle(struct sk_buff* skb,int proto)
{
    ...
    //fast leave enable
	if (port_conf->fastleave || XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_VEIP) //veip is fastleave in xpon_igmp
	{
		del_timer(&entry->leave_ageing_timer);
	...
	else
	{
        ...
        //leave count == 0 ,send leave pkt to wan
	        if(0 < entry->leave_count)
	        {
	            ...
	            XPON_SKB_CB(skb)->pon_vlan_flag |= PON_LEAVE_PKT_DEAL;
...
int xpon_igmp_acl_filter(struct sk_buff* skb){
	...
	gem_port_id = XPON_SKB_CB(skb)->gem_port; 
...
int xpon_igmpv3_pkt_handle(struct sk_buff* skb)
{
	...
	if(portEntry == NULL){
		MULTICAST_NOTICE_INFO("portEntry is NULL,port_name=%s,skb->mark=0x%x,pon_vlan_flag=%d\n",
			port_name,skb->mark,XPON_SKB_CB(skb)->pon_vlan_flag);
		goto skb_drop;
	}
...
static void get_port_from_mark(struct sk_buff* skb, char *port_name)
{
    ...
    MULTICAST_CRITIC_INFO("pon_vlan_flag=%x,skb->mark=%x\n", XPON_SKB_CB(skb)->pon_vlan_flag,skb->mark);
	if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_VEIP)
	{
...
static int frame_is_legal_multicast_pkt(struct sk_buff* skb, int direct) 
{
    ...
    if(MULTICAST_UP_STREAM == direct)
    {
        ...
        if(!(XPON_SKB_CB(skb)->pon_vlan_flag & (PON_PKT_FROM_LAN |PON_PKT_FROM_HYBRID_PPTP | PON_PKT_FROM_HYBRID_VEIP)))
        {
            MULTICAST_CRITIC_INFO("fram is not from uni/veip port. XPON_SKB_CB(skb)->pon_vlan_flag=0x%x\n",XPON_SKB_CB(skb)->pon_vlan_flag);
            if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_VLAN_TX_CALL_HOOK)
            {
...
static char * xpon_get_ingress_dev_info(struct sk_buff* skb)
{
    ...
    return skb->dev ? skb->dev->name : NULL;
#endif
    return NULL;
}
...
int xpon_up_igmp_incoming_hook(struct sk_buff* skb,int clone)
{
	...
	if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_HYBRID_VEIP) //hybride hgu port
	{
...
int xpon_hybrid_down_igmp_incoming_hook(struct sk_buff* skb,int clone)
{
	...
	MULTICAST_DEBUG_INFO(" enter,XPON_SKB_CB(skb)->pon_vlan_flag=%x\n",XPON_SKB_CB(skb)->pon_vlan_flag);
	if (!xpon_is_multicast_addr(dest))
	{
...
static int xpon_down_igmp_ani_vlan_filter(struct sk_buff* skb)
{
    ...
    currAniPort = XPON_SKB_CB(skb)->v_if;
...
int xpon_up_igmp_uni_vlan_filter(struct sk_buff* skb)
{
    ...
    */
    if(XPON_SKB_CB(skb)->pon_vlan_flag & PON_PKT_FROM_LAN)
    {
        rule.port = 0;
        for(i = 4; i > 0; i--)
        {
...
int xpon_up_igmp_ani_vlan_filter(struct sk_buff* skb)
{
    ...
    rule.port = XPON_SKB_CB(skb)->v_if;
